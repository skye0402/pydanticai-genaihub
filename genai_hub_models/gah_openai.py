...
class OpenAIAgentModel(AgentModel):
    ...
    async def _completions_create(
        ...
        # Map messages and flatten any lists
        mapped_messages = []
        for m in messages:
            result = self._map_message(m)
            if isinstance(result, list):
                mapped_messages.extend(result)
            else:
                mapped_messages.append(result)
    ...
    @classmethod
    def _map_user_message(cls, message: ModelRequest) -> list[chat.ChatCompletionMessageParam]:
        messages = []
        for part in message.parts:
            if isinstance(part, SystemPromptPart):
                ...
                raise UnexpectedModelBehavior(f'Unexpected part type in user message: {type(part)}')
        return messages

    @classmethod
    def _map_message(cls, message: ModelMessage) -> Union[chat.ChatCompletionMessageParam, list[chat.ChatCompletionMessageParam]]:
        if isinstance(message, ModelRequest):
            return cls._map_user_message(message)
        ...
        match message:
            ...
            case ToolCallPart():
                return {
                    'role': 'assistant',
                    'content': None,
                    'tool_calls': [_map_tool_call(message)],
                }
            case ToolReturnPart():
                return {
                    'role': 'tool',
